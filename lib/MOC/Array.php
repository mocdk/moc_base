...
class MOC_Array {
    /**
     * Remove all items from $source that does not have a key 
     * matching values in $allowedKeys
     * 
     * @param array $source
     * @param array $allowedKeys
     * @return array
     */
    public static function filterKeys($source, $allowedKeys) {
        foreach ($source as $k => $v) {
            if (false === array_search($k, $allowedKeys)) {
                unset($source[$k]);
            }
        }
        return $source;
        ...
        while (($arg = next($args)) !== false) {
            foreach ((array )$arg as $key => $val) {
                if (is_array($val) && isset($r[$key]) && is_array($r[$key])) {
                    $r[$key] = MOC_Array::merge($r[$key], $val);
                } elseif (is_int($key)) {
                    $r[] = $val;
                } else {
                    $r[$key] = $val;
    ...
    public static function filter($var, $isArray = false) {
        if (is_array($var) && (!empty($var) || $isArray)) {
            return array_filter($var, array('MOC_Array', 'filter'));
        }
        ...
        if (!empty($array) && !empty($array2)) {
            foreach ($array2 as $key => $value) {
                if (!array_key_exists($key, $array)) {
                    ...
                    if (is_array($value)) {
                        $array[$key] = MOC_Array::pushDiff($array[$key], $array2[$key]);
                    }
                }
            }
        }
    ...
    public static function map($class = 'stdClass', $tmp = 'stdClass') {
        ...
        return MOC_Array::__map($val, $class);
    }
        ...
        if (is_array($array)) {
            ...
            foreach ($array as $key => $value) {
                ...
                if (is_numeric($key)) {
                    ...
                    $out[$key] = MOC_Array::__map($value, $class);
                    if (is_object($out[$key])) {
                        if ($primary !== true && is_array($value) && MOC_Array::countDim($value, true) === 2) {
                    ...
                    if ($primary === true) {
                        ...
                        foreach ($value as $key2 => $value2) {
                            $out->{$key2} = MOC_Array::__map($value2, true);
                        }
                    } else {
                        if (!is_numeric($key)) {
                            $out->{$key} = MOC_Array::__map($value, true, $key);
                            ...
                            $out->{$key} = MOC_Array::__map($value, true);
                        }
                    }
                } else {
                    $out->{$key} = $value;
    ...
    function numericKeys($array = null) {
    ...
    public static function enum($select, $list = null) {
        ...
        $return = null;
        $list = MOC_Array::normalize($list, false);
     ...
     * @param array		$keys An array containing one or more MOC_Array::extract()-style key paths
    ...
    public static function format($data, $format, $keys) {
        ...
        for ($i = 0; $i < $count; $i++) {
            $extracted[] = MOC_Array::extract($data, $keys[$i]);
        }
        $out = array();
        $data = $extracted;
        $count = count($data[0]);
    ...
    /**
     * Implements partial support for XPath 2.0. If $path is an array or $data is empty it the call is delegated to MOC_Array::classicExtract.
     *
    ...
    public static function extract($path, $data = null, $options = array()) {
        ...
        if (strpos($path, '/') === false) {
            return MOC_Array::classicExtract($data, $path);
        }
        if (empty($data)) {
            return array();
        }
        ...
        if (!isset($contexts[0])) {
            ...
            if ((is_array($current) && count($data) <= 1) || !is_array($current) || !MOC_Array::numeric(array_keys($data))) {
                $contexts = array($data);
            }
        }
            ...
            if ($conditions) {
                foreach ($conditions as $condition) {
                    ...
                    foreach ($matches as $i => $match) {
                        if (MOC_Array::matches(array($condition), $match['item'], $i + 1, $length)) {
                            $filtered[] = $match;
                        }
                    }
                    $matches = $filtered;
    ...
    public static function matches($conditions, $data = array(), $i = null, $length = null) {
        ...
        if (is_string($conditions)) {
            return !!MOC_Array::extract($conditions, $data);
        }
        foreach ($conditions as $condition) {
            if ($condition === ':last') {
                if ($i != $length) {
                ...
                foreach ($data as $j => $val) {
                    if (is_int($j)) {
                        ...
                        if (empty($tmpPath)) {
                            ...
                            $tmp[] = MOC_Array::classicExtract($val, $tmpPath);
                        }
                    }
                }
                return $tmp;
                ...
                foreach ($data as $j => $val) {
                    if (preg_match('/^' . $pattern . '/s', $j) !== 0) {
                        ...
                        if (empty($tmpPath)) {
                            ...
                            $tmp[$j] = MOC_Array::classicExtract($val, $tmpPath);
                        }
                    }
                }
                return $tmp;
        ...
        if ($all) {
            ...
            if (is_array($array) && reset($array) !== false) {
                foreach ($array as $value) {
                    $depth[] = MOC_Array::countDim($value, true, $count + 1);
                }
            }
            $return = max($depth);
        } else {
            if (is_array(reset($array))) {
                $return = MOC_Array::countDim(reset($array)) + 1;
            } else {
                $return = 1;
            }
        }
    ...
    public static function normalize($list, $assoc = true, $sep = ',', $trim = true) {
        if (is_string($list)) {
            ...
            if ($assoc) {
                return MOC_Array::normalize($list);
            }
     ...
     * to null (useful for MOC_Array::merge). You can optionally group the values by what is obtained when
    ...
    public static function combine($data, $path1 = null, $path2 = null, $groupPath = null) {
        ...
        if (is_array($path1)) {
            ...
            $keys = MOC_Array::format($data, $format, $path1);
        } else {
            $keys = MOC_Array::extract($data, $path1);
        }

        if (!empty($path2) && is_array($path2)) {
            $format = array_shift($path2);
            $vals = MOC_Array::format($data, $format, $path2);

        } elseif (!empty($path2)) {
            $vals = MOC_Array::extract($data, $path2);
        ...
        if ($groupPath != null) {
            $group = MOC_Array::extract($data, $groupPath);
            if (!empty($group)) {
                $c = count($keys);
                for ($i = 0; $i < $c; $i++) {
                    if (!isset($group[$i])) {
        ...
        if (is_a($object, 'XmlNode')) {
            ...
            if (is_object($object)) {
                ...
                foreach ($keys as $key => $value) {
                    if (is_array($value)) {
                        $new[$key] = (array )MOC_Array::reverse($value);
                    } else {
                        if (isset($value->_name_)) {
                            $new = array_merge($new, MOC_Array::reverse($value));
                        } else {
                            $new[$key] = MOC_Array::reverse($value);
                        }
                    }
                }
                if (isset($identity)) {
                ...
                foreach ($object as $key => $value) {
                    $out[$key] = MOC_Array::reverse($value);
                }
            } else {
                $out = $object;
            }
    ...
    public static function flatten($data, $separator = '.') {
        ...
        foreach ($data as $key => $val) {
            if (is_array($val)) {
                $result += (array )MOC_Array::flatten($val, array('separator' => $separator, 'path' => $path . $key));
            } else {
                $result[$path . $key] = $val;
            }
        }
    ...
    public static function __flatten($results, $key = null) {
        ...
        foreach ($results as $k => $r) {
            ...
            if (is_array($r) && count($r)) {
                $stack = array_merge($stack, MOC_Array::__flatten($r, $id));
            } else {
    ...
    public static function sort($data, $path, $dir) {
        $result = MOC_Array::__flatten(MOC_Array::extract($data, $path));
        list($keys, $values) = array(MOC_Array::extract($result, '{n}.id'), MOC_Array::extract($result, '{n}.value'));
